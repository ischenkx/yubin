...
func (s *Publications) Get(id string) (yubin.Publication, bool, error) {
	sql := `
	...
	if err != nil {
		return yubin.Publication{}, false, err
	}
	defer result.Close()
	if !result.Next() {
		return yubin.Publication{}, false, nil
	}
	var publication yubin.Publication
	if err := result.Scan(
		&publication.ID,
...
func (s *Publications) Query(query *crud.Query) ([]yubin.Publication, error) {
	sql := `
	...
	var publications []yubin.Publication
	for result.Next() {
		var publication yubin.Publication
		if err := result.Scan(
			&publication.ID,
...
func (s *Publications) Create(publication yubin.Publication) (yubin.Publication, error) {
	sql := `
	...
	if _, err := s.conn.Exec(context.TODO(), sql,
		...
		return yubin.Publication{}, err
	}
	return publication, nil
}

func (s *Publications) Update(publication yubin.Publication) error {
	sql := `
		UPDATE publications
		SET 
		    topics=$2, 
